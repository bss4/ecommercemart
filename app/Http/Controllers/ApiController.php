    ...
    public function apiRegister(Request $request)
    {
        ...
        if($validator->fails()){
            ...
            if(!empty($request->provider_id))
            {
                ...
                if($Sellers_details)
                {
                	$store_data = Stores::where('seller_id',$Sellers_details->id)->first();
                    $success['flag'] = 'Already exist';
                    $success['store_type'] = isset($store_data->store_type)?$store_data->store_type:'';
                    $success['business_name'] = isset($store_data->business_name)?$store_data->business_name:'';
                }else
                {
    ...
    function otpVarify(Request $request)
    {
        ...
        if($usertemp)
        {
            ...
            }else if(empty($SellerStaff) && !empty($userdata))
            {
            	...
            	$store_data = Stores::where('seller_id',$seller_id)->first();
                $success['store_type'] = isset($store_data->store_type)?$store_data->store_type:'';
                $success['business_name'] = isset($store_data->business_name)?$store_data->business_name:'';
            }
